...
useEffect(()=>{
  setItem(dummyConnectionsData[1]);
for(let i=0;i<dummyPostsData.length;i++)
{
  if(dummyPostsData[i].likes_count.length!=0)
  {
  setLikes(likes+dummyPostsData[i].likes_count.length)
  }
}
  setLoad(false);
...
if(load) {
  ...
  <img className='w-40 h-40 rounded-full border-4 border-amber-50' src={item.profile_picture} alt="" />
        </div>
        <div className='max-lg:ml-0 max-lg:mt-15 p-3 ml-50'>
          <div className=''>
    <h1 className='font-semibold text-xl flex gap-1'>{item.full_name}{item.is_verified && <BadgeCheck className='fill-blue-600 text-white'/>}</h1>
  <p className='text-gray-500'>@{item.username}</p>
          ...
          <div>
            ...
            <div className='mt-5 flex  gap-4 text-gray-700 max-sm:flex-col'>
              <button className=' gap-1 flex rounded-xl  items-center justify-center px-3 py-0.5 text-sm ' ><MapPin/>{item.location}</button>
            ...
            <div className='flex gap-4 mt-2 text-gray-800'>
              <h1 className='flex gap-1'>{dummyPostsData.length} <span className='font-semibold'>Posts</span></h1>
        ...
        <div className=' p-3 max-lg:w-full '>
          <button onClick={()=>{
  setEdit(true)
          }} className='flex max-lg:w-full hover:bg-red-600 transition-all duration-200 rounded-lg hove:shadow px-4 py-1 hover:text-white cursor-pointer border border-gray-500 hover:border-none  gap-2 justify-center items-center'> <Edit/>Edit</button>
        </div>
      </div>
    </div>
...
{
  dummyPostsData.length==0 && <p className='text-gray-600'>No post found!!</p>
}
{

  dummyPostsData.map((post,index)=>(
...
{
  dummyPostsData.length==0 && <p className='text-gray-600'>No media found!!</p>
}
{
